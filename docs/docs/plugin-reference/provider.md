...
支持的数据 provider 既可以被 matcher 直接通过 `"$tag"` 引用，也可以继续被 `domain_set` / `ip_set` 聚合，只要它们具备对应的域名或 IP 匹配能力。少数 provider 只支持直接 matcher 引用，不支持被 `domain_set` / `ip_set` 展平聚合。
...
- 由于 `@@`、`important`、`denyallow` 和 `dnstype` 的优先级需要运行时求值，`adguard_rule` 不能作为 `domain_set.sets` 的来源被展平聚合。
