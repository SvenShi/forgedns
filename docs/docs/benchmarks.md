...
本文档收纳 README 中移出的性能说明，并按版本保留公开基准快照。目标不是宣称绝对胜负，而是展示 OxiDNS 在不同策略复杂度、并发水平和传输路径下的性能轮廓。
...
* 如果你的策略依赖复杂规则集、多数据集匹配，或者经常做多上游并发竞争，`v0.3.0` 下 OxiDNS 的整体表现更有优势；`domain set`、`composite provider chain` 和 `concurrent upstreams` 都明显领先。
