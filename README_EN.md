...
- 📈 Health checks, full and provider-scoped hot reload, config validation, and Prometheus metrics support
...
| Executors | `forward`, `cache`, `fallback`, `hosts`, `arbitrary`, `redirect`, `ecs_handler`, `ttl`, `download`, `upgrade`, `reload`, `reload_provider`, `script`, `http_request` |
