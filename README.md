...
项目受 mosdns 启发，底层建立在 Tokio 与自研 DNS 消息层之上，消息模型与 wire 编解码分层实现，整个设计围绕一句话展开：
...
>
> ForgeDNS 关注的，不只是把请求转发出去，而是让 DNS 在承担缓存、过滤、回退、重写、本地应答、系统联动这些职责时，依然保持足够好的延迟、吞吐和可维护性。

| 维度 | ForgeDNS 的关注点 |
| --- | --- |
| ⚡ 性能 | 尽量缩短热路径，避免随着功能增加而被系统开销吞掉 |
| 🧩 编排 | 用统一的 matcher / executor / provider 管线表达策略 |
...
- `server` 负责协议接入
- `message` 提供自研 DNS 消息模型、RDATA 模型与 wire 编解码
- `network` 负责上游传输、连接池、bootstrap 与不同协议连接复用
- `sequence` 负责策略编排
- `matcher` 负责条件判断
- `executor` 负责执行动作
...
ForgeDNS 当前没有把整个 DNS 数据路径完全建立在第三方消息对象之上，而是维护了一套自研消息层。
这不是为了“重复造轮子”，而是为了让消息模型、wire 编解码、性能优化和策略层需求可以一起演进。

它当前的结构可以理解为两层：

- 一层是 `Message`、`Name`、`Record`、`RData` 这类面向策略管线的 owned model
- 一层是 `message/wire` 下的编解码、压缩、长度估算、截断与 RDATA wire 规则实现

这种分层给 ForgeDNS 带来的直接好处包括：

### 1. 更容易围绕热路径做定向优化

因为消息层是自控的，ForgeDNS 可以直接针对自己的热点做优化，而不必受制于通用库的对象布局和抽象边界。

例如当前已经落地的方向就包括：

- `Name` 同时保留适合编解码的 wire 表示与延迟构建的展示形式
- `TXT`、`OPT`、压缩表、长度估算等路径可以按 ForgeDNS 的请求模型单独优化
- `truncate`、`EDNS`、扩展 `rcode` 等行为可以围绕真实服务路径调整和验证

对于一个强调缓存、回退、重写、本地应答和系统联动的 DNS 服务器来说，这种可控性非常重要。

### 2. 更适合策略层直接操作消息

ForgeDNS 的 `executor` 和 `matcher` 并不是只读 DNS 包，它们经常需要：

- 读取 `qname` / `qtype` / `qclass`
- 修改 `rcode`
- 构造本地 answer
- 保留或回填 `EDNS`
- 对 response 做重写、截断、回退或再加工

如果消息模型和策略层之间隔着一层不完全匹配的外部抽象，很多路径会变得更绕，甚至需要频繁转换对象。
当前这套消息层可以让策略逻辑直接面向 ForgeDNS 自己的语义模型工作，减少不必要的适配成本。

### 3. 更容易把协议正确性和服务语义一起收口

DNS 服务端真正难的地方，往往不只是“能不能把包解出来”，而是：

- 截断行为是否符合协议预期
- `OPT`、`TSIG`、`SIG0` 的位置和保留策略是否正确
- `SVCB/HTTPS`、`NSEC/NSEC3`、`EDNS option` 这些复杂结构是否严格遵守 wire 规则
- 长度估算、压缩和最终编码结果是否一致

自研消息层让这些问题可以直接在同一套代码里收口：

- model 层表达服务语义
- wire 层表达协议规则
- 测试可以同时覆盖字节级 roundtrip 与高层消息行为

这比“策略层用一套对象、编解码再绕到另一套对象”更容易长期保持一致。

### 4. 更容易按 ForgeDNS 的插件化架构继续生长

ForgeDNS 的核心不是一个单纯的 resolver，而是一个围绕 `server -> context -> matcher/executor/provider -> upstream` 展开的策略系统。
这意味着消息层必须能够长期支撑：

- 更多 executor 对 response 的加工
- 更多 matcher 对 query/answer 内容的读取
- 更多系统联动插件对消息结果的观察和提取
- 更多复杂 RDATA 和 EDNS 语义的扩展

拥有自己的消息层之后，这些能力可以围绕现有架构自然生长，而不是不断去迁就某个外部库最初的对象边界。

### 5. 更方便做细粒度测试与基准

现在 `message` 包已经可以独立做：

- `Name` / `RData` / `Message` 的字节级 roundtrip 测试
- `SVCB`、`NSEC`、`EDNS`、`truncate` 等专项协议测试
- `message_codec` benchmark

这让 ForgeDNS 可以把“消息层正确性”和“消息层性能”都作为独立维度持续迭代，而不是只能在整条请求链路里被动观察。

从工程角度看，这是一种长期收益很高的投入：
消息层不只是为了今天能跑，而是为了让整个 DNS 核心在继续扩展时，依然保有性能和正确性的主动权。
...
下面这组数据来自一套外部 `dnsperf` 压测，主要用来展示 ForgeDNS 在不同策略复杂度下的大致性能轮廓，而不是宣称某个绝对“冠军数字”。
...
- 时间：2026-03-26
- 被测二进制标识：`forgedns v0.1.0`，mosdns `v5.3.4-0-gb732318`
...
- 在多上游并发、双入口 UDP、DoH 上游和大规模 `domain_set` 匹配这类更贴近真实策略复杂度的场景里，ForgeDNS 展现出了明显优势。
- 在缓存命中、本地应答、基础 `sequence` 和轻量 matcher 这类偏“纯热路径”的场景里，mosdns 目前仍然更快。
- `domain_set` 场景的差距尤其明显，这和 ForgeDNS 当前对 provider 展平、规则集复用以及策略路径组织方式的设计方向是一致的。
- `fallback standby` 场景下 ForgeDNS 还没有拿到优势，这也说明项目仍然有继续优化主备回退链路的空间。

需要注意：

- 这些结果只代表当前这套机器、这组场景和这份配置下的表现，不代表所有部署环境。
- 压测数据来自外部环境，主要用于展示趋势，而不是替代完整、可复现的独立 benchmark 文档。
- 原始结果中 `14-match-qtype` 目前只包含 ForgeDNS 数据，没有对应的 mosdns 样本，因此这里没有放进直接对比表。
...
- 🩺 管理 API、健康检查、配置校验与热重载控制
- 📈 通过 `metrics_collector` 暴露 Prometheus 文本格式指标
...
- `sequence`：策略编排核心
- `forward`：统一上游转发执行器
- `forward_edns0opt`：按策略传递与回填 EDNS0 option
- `cache`：热路径响应缓存与 TTL/负缓存语义
- `fallback`：主备回退编排
- `hosts`、`arbitrary`、`redirect`、`black_hole`：本地应答与重写基础能力
- `dual_selector`、`ecs_handler`、`ttl`：响应整形与策略增强能力
...
除了 DNS 请求链路本身，ForgeDNS 当前还已经具备一套独立的管理 API 能力，用于健康检查、状态观测和运行时控制。

当前内置接口包括：

- `/healthz`、`/readyz`、`/health`：进程与就绪状态检查
- `/control`：应用控制状态查看
- `/shutdown`：请求优雅关闭
- `/reload`、`/reload/status`：配置热重载与状态查询
- `/config/check`、`/config/validate`：配置检查与校验
- `/metrics`：Prometheus 文本格式指标导出

这些接口和 DoH 服务插件不是一回事。
DoH 是对外提供 DNS 服务的入口，管理 API 则是独立的运维控制面。
...
- 继续扩展管理 API 与插件自定义 API 能力

说明：管理 API、健康检查、热重载控制和 Prometheus 文本指标导出已经落地。这里的后续计划，主要是继续扩展这些运维能力，而不是从零开始实现它们。
